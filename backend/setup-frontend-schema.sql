-- Script de configuração do schema para o frontend Next.js
-- Mantém as tabelas do backend existentes e cria as novas para o frontend

-- PASSO 1: Renomear tabelas antigas do backend (se existirem)
ALTER TABLE IF EXISTS "tickets" RENAME TO "tickets_backend";
ALTER TABLE IF EXISTS "messages" RENAME TO "messages_backend";
ALTER TABLE IF EXISTS "attachments" RENAME TO "attachments_backend";
ALTER TABLE IF EXISTS "users" RENAME TO "users_backend";

-- PASSO 2: Remover tabelas antigas do frontend se existirem
DROP POLICY IF EXISTS "Permitir todas operações em usuarios" ON usuarios;
DROP POLICY IF EXISTS "Permitir todas operações em tickets" ON tickets;
DROP POLICY IF EXISTS "Permitir todas operações em emails" ON emails;
DROP POLICY IF EXISTS "Permitir todas operações em anexos" ON anexos;
...
-- PASSO 2: Criar função para atualizar timestamp
...
-- PASSO 3: Criar tabela de usuários (frontend)
...
-- PASSO 4: Criar tabela de tickets (frontend)
...
-- PASSO 5: Criar tabela de emails
CREATE TABLE emails (
  id BIGSERIAL PRIMARY KEY,
...
-- PASSO 6: Criar tabela de anexos
CREATE TABLE anexos (
  id BIGSERIAL PRIMARY KEY,
...
-- PASSO 9: Inserir usuário admin (tomasbalestrin@gmail.com)
-- Senha: 12345678 (hash bcrypt já gerado)
...
-- PASSO 10: Inserir tickets de exemplo
...
-- PASSO 11: Inserir emails de exemplo
...
SELECT 'Tickets criados:' as info, COUNT(*) as total FROM tickets;
SELECT 'Emails criados:' as info, COUNT(*) as total FROM emails;
