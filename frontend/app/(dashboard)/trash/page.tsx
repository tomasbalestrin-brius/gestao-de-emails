...
export default function TrashPage() {
  ...
  const loadEmails = async () => {
    try {
      setLoading(true)
      const data = await emailsService.getByFolder("trash")
      setEmails(data)
    } catch (error) {
      console.error("[v0] Error loading trash emails:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="flex flex-col">
      <Header
        title="Lixeira"
        description="Emails movidos para lixeira"
        action={
      ...
      <div className="flex-1 space-y-6 p-6">
        {loading ? (
          ...
          <div className="grid gap-3">
            {emails.map((email) => (
              <EmailListItem key={email.id} email={email} folder="trash" />
            ))}
          </div>
        )}
      </div>
